...
export class GiftCodesComponent implements OnInit {
  ...
  applyFilters(): void {
    this.filteredCodes = this.giftCodes.filter(code => {
  ...
  generateBulkCampaign(): void {
    console.log('🚀 Bulk generating campaign:', this.bulkGift);
    
    const newCodes: GiftCode[] = [];
    for (let i = 0; i < this.bulkGift.codes; i++) {
      const newCode: GiftCode = {
        id: (this.giftCodes.length + i + 1).toString(),
        code: `${this.bulkGift.campaignName.toUpperCase().replace(/\s/g, '')}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`,
        type: this.bulkGift.type as GiftCode['type'],
        status: 'active',
        createdDate: new Date().toISOString().split('T')[0],
        expiresDate: this.calculateBulkExpirationDate(),
        value: this.getCodeValue(this.bulkGift.type),
        maxUses: 1,
        currentUses: 0
      };
      newCodes.push(newCode);
  ...
  private calculateBulkExpirationDate(): string {
    const date = new Date();
    date.setDate(date.getDate() + parseInt(this.bulkGift.expiration));
    return date.toISOString().split('T')[0];
  }
