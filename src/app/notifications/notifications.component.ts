...
export class NotificationsComponent implements OnInit {
  ...
  openScheduleModal(notification: Notification): void {
  ...
  getAverageOpenRate(): number {
    ...
    const totalPossible = sentNotifications.length * 1000;
  ...
  scheduleNotification(): void {
    ...
    if (this.selectedNotification) {
      console.log('⏰ Scheduling notification:', this.selectedNotification, this.scheduleDate);
      ...
      alert(`⏰ NOTIFICATION SCHEDULED!\n\nDelivery: ${new Date(this.scheduleDate).toLocaleString()}\nAudience: ${this.getAudienceDisplayName(this.selectedNotification.audience)}\nReady for automated sending!`);
    }
  }
  ...
  createNewCampaign(): void {
    if (!this.newCampaign.name || !this.newCampaign.startDate || !this.newCampaign.endDate) {
    ...
    alert(`🎯 CAMPAIGN CREATED: ${newCampaign.name}\n\nBudget: $${newCampaign.budget}\nDuration: ${newCampaign.startDate} to ${newCampaign.endDate}\nReady to add notifications!`);
  }
